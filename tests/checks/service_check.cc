...
TEST_F(ServiceCheck, SimpleCheck) {
  ...
  std::string cmd{fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;2;service critical", now)};
  ...
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;1;service warning", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ASSERT_EQ(_svc->get_state_type(), checkable::soft);
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;2;service critical", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;0;service ok", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ASSERT_EQ(_svc->get_state_type(), checkable::hard);
...
TEST_F(ServiceCheck, OkCriticalStalking) {
  ...
  time_t now = std::time(nullptr);
  std::string cmd{fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;0;RAID array optimal", now)};
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;0;RAID array optimal", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ...
  for (int i = 0; i < 3; i++) {
    ...
    now = std::time(nullptr);
    cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;1;RAID array degraded (1 drive bad, 1 hot spare rebuilding)", now);
    process_external_command(cmd.c_str());
    checks::checker::instance().reap();
  }
  ASSERT_EQ(_svc->get_state_type(), checkable::hard);
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;2;RAID array degraded (2 drives bad, 1 host spare online, 1 hot spare rebuilding)", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;2;RAID array degraded (3 drives bad, 2 hot spares online)", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
  ...
  now = std::time(nullptr);
  cmd = fmt::format("[{}] PROCESS_SERVICE_CHECK_RESULT;test_host;test_svc;2;RAID array failed", now);
  process_external_command(cmd.c_str());
  checks::checker::instance().reap();
